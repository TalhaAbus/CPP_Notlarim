...
# Statik Yerel Sınıf Nesneleri

- C++ dilinde, statik yerel sınıf nesneleri, bir fonksiyonun kapsamı içinde tanımlanan ve statik ömürlü sınıf nesneleridir. Bu nesneler, fonksiyonun her çağrısı sırasında yeniden başlatılmaz, bunun yerine programın tüm süresi boyunca mevcut olurlar. Statik yerel sınıf nesneleri, fonksiyonlar arasında bilgi saklamak ve paylaşmak için kullanılabilir.

- Statik yerel sınıf nesnelerinin özellikleri şunlardır:

1. **Ömür boyu:** Statik yerel sınıf nesneleri, programın tüm süresi boyunca mevcut olan ömürlüdür. Bunun anlamı, fonksiyonun çağrıldığı süre boyunca ve fonksiyon sona erdikten sonra da nesnenin değeri korunur.

```CPP
void myFunction() {
    static MyClass obj; // Statik yerel sınıf nesnesi
    // ...
}

```

2. **Başlatma:** Statik yerel sınıf nesneleri, ilk kez tanımlandıkları fonksiyonun çağrılması sırasında başlatılır. Başlatma sadece bir kez gerçekleşir ve nesne, programın süresi boyunca geçerli olur.

```CPP
void myFunction() {
    static MyClass obj; // Statik yerel sınıf nesnesi, fonksiyon ilk kez çağrıldığında başlatılır.
    // ...
}

```
3. **Başlatma sırası:** Statik yerel sınıf nesneleri, fonksiyonun ilk çağrısı sırasında başlatılır. Başlatma sırası, fonksiyonun çağrılmasıyla belirlenir ve bu nedenle, farklı fonksiyonlarda tanımlanan statik yerel nesnelerin başlatma sırası belirsizdir.
4. **Erişim:** Statik yerel sınıf nesneleri, tanımlandıkları fonksiyonun kapsamı içinde erişilebilirdir ve sadece bu fonksiyonun içinde kullanılabilirler. Bu, statik yerel nesnelerin, fonksiyonlar arasında bilgi saklamak ve paylaşmak için kullanılabilmesini sağlar.
5. **İplik güvenliği (Thread safety):** C++11'den itibaren, statik yerel sınıf nesnelerinin başlatılması iplik güvenli olarak kabul edilir. Yani, eş zamanlı olarak çalışan birden fazla iplik (thread) içinde aynı fonksiyona erişilse bile, statik yerel nesnelerin başlatılması doğru bir şekilde senkronize edilir.

- Statik yerel sınıf nesneleri, fonksiyonlar arasında bilgi saklamak ve paylaşmak için kullanılabilen ve programın süresi boyunca mevcut olan sınıf nesneleridir. Statik yerel nesnelerin kullanımı, özellikle kaynakları paylaşmak, global değişkenlerin yerini almak veya tekil yapıları (singleton) uygulamak için kullanışlıdır.

- Örnek olarak, bir sayacı yöneten bir fonksiyon düşünelim:

```CPP
int counter() {
    static int count = 0; // Statik yerel sınıf nesnesi
    count++; // Her fonksiyon çağrısında sayacı artır
    return count;
}
